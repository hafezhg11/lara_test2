...
<section class="content-header">
    <h1>
        {{ __('action.Actions') }}
        <small>{{__('action.View All Actions')}}</small>
    </h1>
    <ol class="breadcrumb">
...
@foreach ($actions as $action)
    <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12" @if(!\App::isLocale('en')) style="float: right" @endif>
        ...
        <div class="panel panel-info">
            ...
            {{__('action.Action Name')}}: {{$action->name}}
            </div>
            <div class="panel-body">
                <p>{{ __('action.Action Name') }}:{{$action->name}}</p>
{{--                <p>{{ __('operation.Specification') }}:{{$operation->specification}}</p>--}}
            ...
            <div class="panel-footer" >
                ...
                <a title="{{__("action.Show")}}" data-toggle="modal" data-target="#show{{$action->id}}" type="button" class="btn btn-sm btn-primary" ><i class="fa fa-list"></i></a>
                <span class="pull-right">
                    <a href="<?= url('action/'.$action->id.'/edit'); ?>" data-original-title="{{__("action.Edit")}}" data-target="#{{$action->id}}" type="submit" class="btn btn-sm btn-warning"><i class="fa fa-edit"></i></a>
                    <a data-original-title="{{__("action.Delete")}}" data-toggle="modal" data-target="#delete{{$action->id}}" type="button" class="btn btn-sm btn-danger"><i class="fa fa-remove"></i></a>
                </span>
            </div>
        </div>
    ...
    {{--delete modal--}}
        <div class="modal fade" id="delete{{$action->id}}" tabindex="-1" role="dialog" aria-labelledby="mylable" aria-hidden="true">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="mymodallabel">
                            {{__('action.Action Delete')}}
                        </h4>
                        <form action="<?= url('action/'.$action->id) ?>" method="POST">
                            <input type="hidden" name="_token" value="{{csrf_token()}}">
                            <input type="hidden" name="_method" value="DELETE">
                            <div class="modal-body">
                                <h3>{{__('action.Are you sure you want to delete the following Action?')}} <hr>
                                     {{$action->name}}</h3>
                            </div>
                            <div class="modal-footer">
                                {{--<button type="button" class="btn btn-danger">حذف</button>--}}
                                <input type="submit" class="btn btn-danger" value="{{__('action.Delete')}}">
    ...
    {{-- /. delete modal--}}
        {{--show modal--}}
        <div class="modal" id="show{{$action->id}}" tabindex="-1" role="dialog" aria-labelledby="mylable" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
