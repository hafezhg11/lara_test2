...
@section('content')
    <section class="content-header">
        <h1>
            {{ __('action.Actions') }}
            <small>{{__('action.Create New Action')}}</small>
        </h1>
        <ol class="breadcrumb">
...
{{--                    {{Form::lable('firstname',['class'=>'control-lable'])}}--}}
    ...
    <div class="col-md-12 center-block">
        ...
        <div class="box box-primary">
            ...
            {{--<form role="form">--}}
{{--            {{ Form::open(['url'=>'user','class'=>'form-horizontal']) }}--}}
            {{ Form::open(['url'=>'action/store','class'=>'']) }}
                <div class="box-body">
                    <div class="form-group">
                        <label for="exampleInputEmail1">{{__('action.Action Name')}}</label>
                        <input name="name" class="form-control" id="" placeholder="{{__('action.Action Name')}}" type="text">
                    </div>
                    <div class="form-group">
                        <label>{{__('action.Operation Name')}}</label>
                        <select class="form-control" name="operation_id">
                            <option value="" selected>{{__('action.Choose A Operation')}}</option>
                    ...
                    <div class="form-group">
                        <div class="radio">
                            <label><input type="radio" name="status" value="1" checked>{{__('action.Enable')}}</label>
                        </div>
                        <div class="radio">
                            <label><input type="radio" name="status" value="0">{{__('action.Disable')}}</label>
                        </div>
                    </div>
                    {{--******************--}}
                    {{--<div class="checkbox">--}}
