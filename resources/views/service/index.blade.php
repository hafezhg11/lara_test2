...
<section class="content-header">
    <h1>
        {{ __('service.Services') }}
        <small>{{__('service.View All Services')}}</small>
    </h1>
    <ol class="breadcrumb">
...
@foreach ($services as $service)
    <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12" @if(!\App::isLocale('en')) style="float: right" @endif>
        ...
        <div class="panel panel-info">
            ...
            {{__('service.Service Name')}}: {{$service->name}}
            </div>
            <div class="panel-body">
                <p>{{ __('service.Service Name') }}:{{$service->name}}</p>
                {{--<p>{{ __('service.Branch Name') }}:{{$service->load('branches.branch.title')}}</p>--}}
                <p>{{ __('service.Specification') }}:{{$service->specification}}</p>
            </div>
            ...
            <div class="panel-footer" >
                ...
                <a title="{{__("service.Show")}}" data-toggle="modal" data-target="#show{{$service->id}}" type="button" class="btn btn-sm btn-primary" ><i class="fa fa-list"></i></a>
                <span class="pull-right">
                    <a href="<?= url('service/'.$service->id.'/edit'); ?>" data-original-title="{{__("service.Edit")}}" data-target="#{{$service->id}}" type="submit" class="btn btn-sm btn-warning"><i class="fa fa-edit"></i></a>
                    <a data-original-title="{{__("service.Delete")}}" data-toggle="modal" data-target="#delete{{$service->id}}" type="button" class="btn btn-sm btn-danger"><i class="fa fa-remove"></i></a>
                </span>
            </div>
        </div>
    ...
    {{--delete modal--}}
        <div class="modal fade" id="delete{{$service->id}}" tabindex="-1" role="dialog" aria-labelledby="mylable" aria-hidden="true">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="mymodallabel">
                            {{__('service.Service Delete')}}
                        </h4>
                        <form action="<?= url('service/'.$service->id) ?>" method="POST">
                            <input type="hidden" name="_token" value="{{csrf_token()}}">
                            <input type="hidden" name="_method" value="DELETE">
                            <div class="modal-body">
                                <h3>{{__('service.Are you sure you want to delete the following service?')}} <hr>
                                     {{$service->name}}</h3>
                            </div>
                            <div class="modal-footer">
                                {{--<button type="button" class="btn btn-danger">حذف</button>--}}
                                <input type="submit" class="btn btn-danger" value="{{__('service.Delete')}}">
    ...
    {{-- /. delete modal--}}
        {{--show modal--}}
        <div class="modal" id="show{{$service->id}}" tabindex="-1" role="dialog" aria-labelledby="mylable" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
                        ...
                        <div class="modal-body">
                            <p>{{__('service.Service Name')}}: {{$service->name}}</p>
{{--                            <p>{{__('service.Branch Name')}}: {{$service->branch_id}}</p>--}}
                            <p>{{__('service.Branch Name')}}: {{$branches->find($service->branch_id)->title}}</p>
                            <p>{{__('service.Specification')}}: {{$service->specification}}</p>
                            <p>{{__('service.Average Processing Time')}}: {{$service->ave_processing_time}}</p>
                        </div>
                    </div>
                </div>
            </div>
