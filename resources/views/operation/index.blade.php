...
<section class="content-header">
    <h1>
        ...
        <small>{{__('operation.View All Operations')}}</small>
    </h1>
    <ol class="breadcrumb">
...
@foreach ($operations as $operation)
    <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12" @if(!\App::isLocale('en')) style="float: right" @endif>
        ...
        <div class="panel panel-info">
            ...
            {{__('operation.Operation Name')}}: {{$operation->name}}
            </div>
            <div class="panel-body">
                <p>{{ __('operation.Operation Name') }}:{{$operation->name}}</p>
{{--                <p>{{ __('operation.Specification') }}:{{$operation->specification}}</p>--}}
            </div>
            ...
            <div class="panel-footer" >
                ...
                <a title="{{__("operation.Show")}}" data-toggle="modal" data-target="#show{{$operation->id}}" type="button" class="btn btn-sm btn-primary" ><i class="fa fa-list"></i></a>
                <span class="pull-right">
                    <a href="<?= url('operation/'.$operation->id.'/edit'); ?>" data-original-title="{{__("operation.Edit")}}" data-target="#{{$operation->id}}" type="submit" class="btn btn-sm btn-warning"><i class="fa fa-edit"></i></a>
                    <a data-original-title="{{__("operation.Delete")}}" data-toggle="modal" data-target="#delete{{$operation->id}}" type="button" class="btn btn-sm btn-danger"><i class="fa fa-remove"></i></a>
                </span>
            </div>
        </div>
    ...
    {{--delete modal--}}
        <div class="modal fade" id="delete{{$operation->id}}" tabindex="-1" role="dialog" aria-labelledby="mylable" aria-hidden="true">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="mymodallabel">
                            {{__('operation.Operation Delete')}}
                        </h4>
                        <form action="<?= url('operation/'.$operation->id) ?>" method="POST">
                            <input type="hidden" name="_token" value="{{csrf_token()}}">
                            <input type="hidden" name="_method" value="DELETE">
                            <div class="modal-body">
                                <h3>{{__('operation.Are you sure you want to delete the following operation?')}} <hr>
                                     {{$operation->name}}</h3>
                            </div>
                            <div class="modal-footer">
                                {{--<button type="button" class="btn btn-danger">حذف</button>--}}
                                <input type="submit" class="btn btn-danger" value="{{__('operation.Delete')}}">
    ...
    {{-- /. delete modal--}}
        {{--show modal--}}
        <div class="modal" id="show{{$operation->id}}" tabindex="-1" role="dialog" aria-labelledby="mylable" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
